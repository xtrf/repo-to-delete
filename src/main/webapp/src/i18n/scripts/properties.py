...
class Properties(object):
    ...
    def processPair(self, key, value):
        ...
        self.buildStructure(self._structure, key.split('.'), value.strip())
    ...
    def buildStructure(self, root, remainingNodes, value):
        #print value, remainingNodes, root
        if len(remainingNodes) == 1:
            root[remainingNodes[0]] = value
        else:
            if remainingNodes[0] not in root:
                root[remainingNodes[0]] = {}
            self.buildStructure(root[remainingNodes[0]], remainingNodes[1:], value)
            
    def getStructure(self):
        return self._structure
        
    def fromStructure(self, path, node):
        if type(node) is dict:
            for field in node:
                pathCopy = path[:]
                pathCopy.append(field)
                self.fromStructure(pathCopy, node[field])
        else:
            self.setProperty(".".join(path), node)
    ...
    def store(self, out, header=""):
        ...
        try:
            ...
            #list = self._origprops.items()[:]
            for prop, val in self.getSortedItems():
