...
app.factory('AlertsQueue', ['$translate', function($translate) {
	return function(initQueue) {
		...
		function create(message, options) {
			var scope = {
				message: options && options.html ? message : $translate.instant(message, options && options.translationData),
		...
		function withTitle(alert, title, options) {
			if (title) {
				alert.title = options && options.html ? title : $translate.instant(title, options && options.translationData);
			}
			return alert;
		}
